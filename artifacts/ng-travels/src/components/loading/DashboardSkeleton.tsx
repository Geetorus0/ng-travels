...
export const MetricCardSkeleton: React.FC<{ className?: string }> = ({ className }) => {
  return (
    <div className={cn("p-4 rounded-2xl bg-card/60 border border-border/80 space-y-2", className)}>
  ...
  return (
    <div className="grid grid-cols-2 xs:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5 sm:gap-3">
      {Array.from({ length: 12 }).map((_, i) => (
        <div
          key={i}
          className="p-3 sm:p-4 rounded-xl bg-card/50 border border-border/70 flex flex-col justify-between min-h-[85px] space-y-2"
        >
...
export const ScheduleRowSkeleton: React.FC = () => {
  return (
    <div className="p-3.5 rounded-xl bg-card/60 border border-border/70 flex items-center justify-between gap-3">
      <div className="space-y-1.5 flex-1">
        <div className="flex items-center gap-2">
          <Skeleton className="h-4 w-20 bg-muted font-mono" />
    ...
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-300">
      ...
      <div className="p-6 rounded-2xl bg-card/60 border border-border flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-2 flex-1">
          <Skeleton className="h-4 w-32 bg-amber-100 dark:bg-amber-400/20" />
          <Skeleton className="h-7 w-64 bg-muted" />
          <Skeleton className="h-3 w-96 max-w-full bg-muted/60" />
        </div>
        <Skeleton className="h-11 w-44 rounded-xl bg-amber-100 dark:bg-amber-400/20" />
      </div>
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-card/60 border border-border space-y-3">
        <div className="flex justify-between items-center">
