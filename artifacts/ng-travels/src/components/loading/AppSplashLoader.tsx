...
export const AppSplashLoader: React.FC<AppSplashLoaderProps> = ({
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 text-foreground selection:bg-amber-400 selection:text-zinc-950 relative overflow-hidden">
      {/* Background ambient lighting */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-amber-100 dark:bg-amber-500/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-sm flex flex-col items-center text-center space-y-6 z-10">
        ...
        <div className="relative group">
          ...
          <img
            src="/logo.png"
            alt="NG Travels"
            className="relative w-24 h-24 rounded-2xl object-contain bg-black p-1.5 border border-amber-300 dark:border-amber-500/40 shadow-2xl shadow-amber-500/20"
          />
        </div>

        {/* Brand Name & Tagline */}
        <div className="space-y-1">
          <h1 className="text-2xl font-black text-foreground tracking-tight flex items-center justify-center gap-1.5">
            <span>NG TRAVELS</span>
            {mode === "driver" ? (
              <span className="text-[10px] bg-sky-100 dark:bg-sky-500/20 text-sky-700 dark:text-sky-300 px-2 py-0.5 rounded-full border border-sky-300 dark:border-sky-500/40 font-mono">
                PILOT
              </span>
            ) : mode === "owner" ? (
              <span className="text-[10px] bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 px-2 py-0.5 rounded-full border border-amber-300 dark:border-amber-500/40 font-mono">
                OWNER
              </span>
            ) : null}
          </h1>
        ...
        {isOffline || isTimedOut ? (
          <div className="bg-card/90 border border-border rounded-2xl p-5 space-y-3 w-full shadow-xl animate-in fade-in zoom-in-95 duration-300">
            <div className="w-10 h-10 rounded-full bg-rose-100 dark:bg-rose-500/10 border border-rose-300 dark:border-rose-500/30 text-rose-700 dark:text-rose-400 flex items-center justify-center mx-auto">
          ...
          <div className="w-full space-y-4 pt-2">
            ...
            <div className="h-6 flex items-center justify-center">
              <p className="text-xs font-bold text-foreground tracking-wide transition-all duration-300 animate-in fade-in">
      ...
      <div className="absolute bottom-6 text-[10px] text-muted-foreground font-mono text-center">
