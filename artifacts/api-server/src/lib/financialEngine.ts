...
/**
 * Authoritative Commercial Fare Engine
 * Evaluates verified road distance against minimum KM rules, applies driver bata,
 * separate permits, real tolls, parking, waiting, taxes, and discounts.
 */
...
export function calculateCompanyProfit(revenue: number, approvedExpenses: number): number {
  return Math.round((Number(revenue || 0) - Number(approvedExpenses || 0)) * 100) / 100;
}
