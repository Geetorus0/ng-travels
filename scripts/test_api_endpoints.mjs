...
async function runTest() {
  console.log("🚀 Launching NG Travels Production Backend for Verification...\n");

  const port = 5099;
  const child = spawn("node", ["./dist/index.mjs"], {
    cwd: "d:/NG-Travels-Operations/artifacts/api-server",
  ...
  try {
    // 1. Health / Vehicles List
    const vRes = await fetch(`${baseUrl}/api/vehicles`);
    const vehicles = await vRes.json();
    assert(vRes.status === 200 && Array.isArray(vehicles) && vehicles.length > 0, "GET /api/vehicles returns fleet list");
    ...
    console.log("locBody output:", locBody);
    assert(locRes.status === 200 && (locBody.success || locBody.point || locBody.latitude), "Telemetry: GPS coordinates ingested successfully");
